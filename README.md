...
这是一个基于 [musicdl](https://github.com/CharlesPikachu/musicdl) 的个人学习项目，用于研究 Python 音乐检索工具、Web 播放界面和本地化配置流程。

本仓库保留 musicdl 的核心能力，并在示例 Web 播放器上做了一些适合自用部署的改造：

- 修复 Web 示例的流式搜索调用兼容问题。
- 增加右上角设置入口，可在本地保存各音乐源的 Cookie 配置。
- 调整部分目录和配置，方便在个人环境中运行。
...
这些文件只用于本地运行环境，已经被 `.gitignore` 排除，不应提交到仓库。
